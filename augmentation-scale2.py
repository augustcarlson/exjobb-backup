...
def augmentation():
    os.mkdir("augmentation2")
    print("Loading parameters...")
    chunkLengthFile = open("augmentation-parameters/chunk-times2.txt")
    chunkLengthFileStr = chunkLengthFile.read()
    chunkLengthFile.close()
    chunkLengths = chunkLengthFileStr.split("\n") 
    chunkLengths.pop(-1)
    interArrivalFile = open("augmentation-parameters/normalized-inter-arrivals2.txt")   
    ...
    for folder in range(100):
        print("Folder", folder)
        os.mkdir("augmentation2/"+str(folder))
            ...
            for i in range(20):#how many times is each trace duplicated and augmented
                for sample in range(10):
                    if folder >= 10:
                        ...
                        if newSample >= 100:
                            f = open("augmentation2/"+str(folder)+"/00"+str(folder)+"-000"+str(trace)+"-0"+str(newSample)+".log", "w+")
                        elif newSample >= 10:
                            f = open("augmentation2/"+str(folder)+"/00"+str(folder)+"-000"+str(trace)+"-00"+str(newSample)+".log", "w+")
                        else:
                            f = open("augmentation2/"+str(folder)+"/00"+str(folder)+"-000"+str(trace)+"-000"+str(newSample)+".log", "w+")
                    else:
                        filename = "../../data/dataset-231121/none/"+str(folder)+"/000"+str(folder)+"-000"+str(trace)+"-000"+str(sample)+".log"
                        
                        if newSample >= 100:
                            f = open("augmentation2/"+str(folder)+"/000"+str(folder)+"-000"+str(trace)+"-0"+str(newSample)+".log", "w+")
                        elif newSample >= 10:
                            f = open("augmentation2/"+str(folder)+"/000"+str(folder)+"-000"+str(trace)+"-00"+str(newSample)+".log", "w+")
                        else:
                            f = open("augmentation2/"+str(folder)+"/000"+str(folder)+"-000"+str(trace)+"-000"+str(newSample)+".log", "w+")
...
def augmentation_spackets():
    ...
    print("Loading parameters...")
    chunkLengthFile = open("augmentation-parameters/chunk-times-scale2.txt")
    chunkLengthFileStr = chunkLengthFile.read()
    chunkLengthFile.close()
    chunkLengths = chunkLengthFileStr.split("\n") 
    chunkLengths.pop(-1)
    interArrivalFile = open("augmentation-parameters/normalized-inter-arrivals-scale2.txt")   
    interArrivalFileStr = interArrivalFile.read()
    interArrivalFile.close()
    interArrivals = interArrivalFileStr.split("\n")
    interArrivals.pop(-1) 
