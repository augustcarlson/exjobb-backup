...
def augmentation():
    os.mkdir("augmentation8")
    print("Loading parameters...")
    chunkLengthFile = open("augmentation-parameters/chunk-times8.txt")
    chunkLengthFileStr = chunkLengthFile.read()
    chunkLengths = chunkLengthFileStr.split("\n") 
    chunkLengths.pop(-1)
    interArrivalFile = open("augmentation-parameters/normalized-inter-arrivals8.txt")   
    ...
    for folder in range(100):
        print("Folder", folder)
        os.mkdir("augmentation8/"+str(folder))
            ...
            for i in range(20):#how many times is each trace duplicated and augmented
                for sample in range(10):
                    ...
                    if folder >= 10:
                        ...
                        if newSample >= 100:
                            f = open("augmentation8/"+str(folder)+"/00"+str(folder)+"-000"+str(trace)+"-0"+str(newSample)+".log", "w+")
                        elif newSample >= 10:
                            f = open("augmentation8/"+str(folder)+"/00"+str(folder)+"-000"+str(trace)+"-00"+str(newSample)+".log", "w+")
                        else:
                            f = open("augmentation8/"+str(folder)+"/00"+str(folder)+"-000"+str(trace)+"-000"+str(newSample)+".log", "w+")
                    else:
                        filename = "../../data/dataset-231121/none/"+str(folder)+"/000"+str(folder)+"-000"+str(trace)+"-000"+str(sample)+".log"
                        
                        if newSample >= 100:
                            f = open("augmentation8/"+str(folder)+"/000"+str(folder)+"-000"+str(trace)+"-0"+str(newSample)+".log", "w+")
                        elif newSample >= 10:
                            f = open("augmentation8/"+str(folder)+"/000"+str(folder)+"-000"+str(trace)+"-00"+str(newSample)+".log", "w+")
                        else:
                            f = open("augmentation8/"+str(folder)+"/000"+str(folder)+"-000"+str(trace)+"-000"+str(newSample)+".log", "w+")
                    ...
                    for line in s:
                        ...
                        if float(parts[0]) < currentChunkEnd:
                            currentChunkSize += float(parts[2])-52#-52Remove packet overhead
                        else:
...
def augmentation_spackets():
    os.mkdir("augmentation")
    print("Loading parameters...")
    chunkLengthFile = open("augmentation-parameters/chunk-times-scale8.txt")
    chunkLengthFileStr = chunkLengthFile.read()
    chunkLengths = chunkLengthFileStr.split("\n") 
    chunkLengths.pop(-1)
    interArrivalFile = open("augmentation-parameters/normalized-inter-arrivals-scale8.txt")   
    interArrivalFileStr = interArrivalFile.read()
    interArrivals = interArrivalFileStr.split("\n")
    interArrivals.pop(-1) 
