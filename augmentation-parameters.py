...
def get_params():
    ...
    for folder in range(100):
        ...
        for trace in range(10):
            for sample in range(3):
                if folder >= 10:
                    filename = "../../data/dataset-240118-0-3/none-none/"+str(folder)+"/00"+str(folder)+"-000"+str(trace)+"-000"+str(sample)+".log"
                else:
                    filename = "../../data/dataset-240118-0-3/none-none/"+str(folder)+"/000"+str(folder)+"-000"+str(trace)+"-000"+str(sample)+".log"
    ...
    file = open("augmentation-parameters/normalized-inter-arrivals.txt", "w+")
    for value in normalizedInterArrivals:
        file.write(str(value))
        file.write("\n")

    chunkLengths.sort()
    file = open("augmentation-parameters/chunk-times.txt", "w+")
